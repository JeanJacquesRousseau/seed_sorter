...
int main(int, char** argv)
{
    ...
    Mat frame_label, frame_gray, frame_threshold;
    Seed seed;
...
//    imshow("Markers_v2", mark); // uncomment this if you want to see how the mark
	...
	cvtColor( dst, frame_gray, COLOR_BGR2GRAY );	//Grey scale convertion
	threshold(frame_gray, frame_threshold, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);  //Otsu thresholding
	//seeds = seed.identifier(frame_gray);
    frame_label = seed.draw(seeds, dst,src);
    //cout << seeds << endl;
    imshow("check moi caaa",frame_label);
    //imshow("identifier input", frame_gray);
    imwrite("graine.jpg",frame_label);
    waitKey(0);
    return 0;
}
