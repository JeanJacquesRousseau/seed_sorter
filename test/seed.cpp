...
vector<Point3f> Seed::identifier(Mat mask){
	...
	for(uint32_t i = 1; i < nbGraine_; i++ ) {	///Assign color in function of Area size of blob
		...
		if( stats_.at<int>(i, cv::CC_STAT_AREA) < 300 ) {	// element i = 0 area is backgrounds area	AREA = 1250 for 800X600 Res
			//colors[i] = cv::Vec3b(0,0,255);	///Soybeans in red
			graines[i-1].z = 1;
		}
		else if( stats_.at<int>(i, cv::CC_STAT_AREA) > 300 ) { //&& stats_.at<int>(i, cv::CC_STAT_AREA) < 2250
